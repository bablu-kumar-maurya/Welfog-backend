...
router.post("/", async (req, res) => {
    try {
        ...
        const usernameRegex = /^(?=.*[a-z])(?=.*[0-9])(?=.*[@$!%*?&_])[a-z0-9@$!%*?&_]{6,20}$/;
    ...
    try {
        ...
        if (!query) {
            const videos = await Reel.find({ status: "Published" })
                .select("userid username name videoUrl thumbnailUrl caption likes views createdAt music")
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        await Promise.all([
            ...
            Reel.updateMany({ 'shares.sharedBy': userId }, { $pull: { shares: { sharedBy: userId } } }),
            Reel.updateMany({ 'shares.sharedTo': userId }, { $pull: { shares: { sharedTo: userId } } }),
        ]);
...
router.delete("/admin_users/:id",adminAuth, checkPermission("DELETE_USER"), async (req, res) => {
    try {
        const userId = req.params.id;
...
router.put("/:id/follow", async (req, res) => {
    ...
    if (userId === req.params.id) return res.status(400).json({ message: "You can't follow yourself" });
...
router.put("/admin_users/:id", adminAuth, checkPermission("SUSPEND_USER"),async (req, res) => {
    const { username, name, email, mobile, profilePicture, bio, currentPassword, newPassword, isSuspended } = req.body;
    ...
    try {
        ...
        if (category === "authentication" || category === "moderation") {
            if (category === "authentication") {
                ...
                logFilter.action = { $in: ["report_reel", "account_suspended", "account_reactivated"] };
            }
        }
        ...
        if (!category || category === "content" || category === "social") {
            ...
            userReels.forEach(reel => {
                ...
                // Updated reels
                if (reel.updatedAt && reel.updatedAt.getTime() !== reel.createdAt.getTime()) {
                    activities.push({
                        type: "reel",
                        category: "content",
        ...
        if (!category || category === "content") {
            const musicUploads = await Music.find({ uploadedBy: userId, ...dateFilter })
                .sort({ createdAt: -1 })
                .limit(100);
        ...
        const summary = {
            ...
            totalLogins: await UserLog.countDocuments({ user: userId, action: "login" })
        };
...
function getCategoryFromAction(action) {
    if (["login", "logout", "account_created", "account_suspended", "account_reactivated"].includes(action)) {
        return "authentication";
    }
    if (["report_reel", "block_reel", "account_suspended", "account_reactivated"].includes(action)) {
        return "moderation";
    }
    if (["upload_reel", "delete_reel", "edit_reel", "upload_music", "delete_music"].includes(action)) {
        return "content";
    }
    if (["like_reel", "unlike_reel", "comment", "reply_comment", "like_comment", "follow_user", "unfollow_user"].includes(action)) {
        return "social";
    }
    return "other";
}
